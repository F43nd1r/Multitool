...
public class BadgeSetup {
    ...
    public BadgeSetup(Lightning lightning) {
        utils = new Utils(lightning);
    }

    public void show() {
        Script create = utils.installScript("badge", R.raw.badge_create, "create");
        ActivityScreen screen = ProxyFactory.cast(utils.getScreen(), ActivityScreen.class);
    ...
    public void create(int resultCode, Intent data, String token) {
        if (resultCode == Activity.RESULT_OK) {
            Container d = utils.getScreen().getContainerById(Integer.parseInt(token));
